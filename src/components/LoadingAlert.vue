<template>
    <v-dialog v-model="alert" max-width="500px" persistent transition="dialog-bottom-transition">
        <v-card>
                <v-container>
                    <v-row align="center" justify="center">
                        <v-col cols="12" class="d-flex justify-center">
                            ...
                            <v-icon v-if="state === 'success'" color="success" large>mdi-check</v-icon>
                            <v-icon v-if="state === 'error'" color="error" large>mdi-close</v-icon>
                        </v-col>
                        <v-col cols="12" class="d-flex justify-center">
                            <p>
                ...
                <v-card-actions v-if="state === 'success' || 'error'">
                    <v-spacer></v-spacer>
                    <v-btn v-if="state === 'success'" text color="primary" @click='emitSaveSuccess'>Close</v-btn>
                    <v-btn v-if="state === 'error'" text color="primary" @click='emitSaveError'>Close</v-btn>
                </v-card-actions>
        </v-card>
    </v-dialog>
</template>
